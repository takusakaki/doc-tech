 ...
 *_****************************************************************************************
  _画面に見えている範囲しか移動できない（org-modeのアウトラインで隠れている箇所も対象外となる）_
  _このため検索というより、画面を複数のフレームで細かく分割しているときの移動に役に立ちそう_
 *_****************************************************************************************
...
** ediff-regions-wordwise, ediff-regions-linewise                    :remark:
 ...
 手順)
  1. M-x ediff-regions-wordwise を実行
  2. 比較対象のバッファを2つ指定 (事前に比較するファイルを開いておく必要あり)
  3. それぞれのバッファでリージョンを選択し、C-M-c
...
: _下付き  _{ここだけ下付き}にする
  ↓
 *ボールド* /イタリック/ _アンダーライン_ =WWWWWiiiiWWW= ~WWWWWiiiiWWW~ +取り消し+
 ^上付き ^{ここだけ上付き}にする
 _下付き _{ここだけ下付き}にする
...
** org-modeでの検索 (org-sparse-tree)                                :remark:
...
** install
package-list-package から helmをインストール
...
いくつかの標準的なコマンドが置き換わる
 ...
 C-x C-b  list-buffers        =>`helm-buffers-list'
 M-tab    completion-at-point =>`helm-lisp-completion-at-point'
 M-s o    occur               =>`helm-occur'
 M-/      dabbrev-expand      =>`helm-dabbrev'

- help
   helm の何らかのコマンドの途中で、C-c ?
...
helm session
  TAB  選択したファイルに対するmenuを開く
  C-z  helm sessionを維持する

*** ファイルを開く  C-x C-f   --> ファイルの候補がバッファに表示される。
 キー入力で絞り込み (SPC区切りで複数)
 C-SPC マーク
 M-a   全選択
 C-j   下位の階層に移動
 ...
 M-x helm-for-files  履歴等からもファイルを探せる


*** バッファ一覧  C-x b , C-x C-b
 helm-buffers-list (C-x C-b) から... 
  ファイル名で絞り込み スペース区切りで複数の条件を指定可能
  '@<pattern>' を入力するとバッファ内の文字列でも絞り込むことも可能

 <tab> でアクションを選択


*** その他
 (helm-command-prefix) M-y  helm-show-kill-ring   kill ring を表示
